    ...
    <Box sx={{ display: 'flex', justifyContent: 'center' }}>
      <svg width="100%" height={110 + 20 * params.length}>
        ...
        <g>
          ...
          <text x="50%" y="50%" dominantBaseline="middle" textAnchor="middle" fontSize="1.1em" fontWeight="bold" fontFamily={theme.custom.monospaceFontFamily} {...hoverProps(nameId(), hoverElement, setHoverElement)}>{name}</text>
        </g>
        {params.map((param, i) => {
          ...
          return (
            ...
            <g key={i} {...hoverProps(paramId(i), hoverElement, setHoverElement)}>
              ...
              <line x1="2%" x2="30%" stroke={DARK_COLOR} strokeWidth="3" y1={`${y}%`} y2={`${y}%`} strokeDasharray={dashed ? '10 4' : 'none'} />
              ...
              <text transform="translate(0, -8)" x="27%" y={`${y}%`} dominantBaseline="text-top" textAnchor="end" fontFamily={theme.custom.monospaceFontFamily}>{param.name}</text>
            </g>
          );
        })}
        {returnValue && (
          <g {...hoverProps(returnId(), hoverElement, setHoverElement)}>
            ...
            <text transform="translate(0, -8)" x="73%" y="50%" dominantBaseline="text-top" textAnchor="start" fontStyle="italic">{returnValue.type}</text>
            )}
          </g>
        )}
        {fn.sideEffects && (
